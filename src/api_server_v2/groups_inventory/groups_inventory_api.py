...
def init_items_table():
    ...
    if not table_exists:
        ...
        # Check if the 'item_order' column exists
        cursor.execute("PRAGMA table_info(items)")
        columns = [column['name'] for column in cursor.fetchall()]
        if 'item_order' not in columns:
            print("Adding 'item_order' column to 'items' table...")
            cursor.execute('ALTER TABLE items ADD COLUMN item_order INTEGER')
            conn.commit()
            print("Column 'item_order' added.")
        else:
            print("Table 'items' already exists with 'item_order' column.")
...
def get_items():
    ...
    cursor = conn.cursor()
    if group_id:
        cursor.execute('SELECT * FROM items WHERE group_id = ? ORDER BY item_order, id', (group_id,))
    else:
        cursor.execute('SELECT * FROM items ORDER BY group_id, item_order, id')
...
def create_item():
    ...
    # Set initial order to be the number of items currently in the group
    cursor.execute('SELECT COUNT(*) FROM items WHERE group_id = ?', (group_id,))
    item_order = cursor.fetchone()[0]

    cursor.execute(
        'INSERT INTO items (length, width, height, note, group_id, item_order) VALUES (?, ?, ?, ?, ?, ?)',
        (length, width, height, note, group_id, item_order)
    )
...
def update_item(item_id):
    ...
    try:
        ...
        height = float(data['height'])
        group_id = int(data['group_id'])
        # The order is typically updated via the sequence API, but we include it here for completeness
    ...
    # We only update item_order if it's explicitly provided
    if item_order is not None:
        cursor.execute(
            'UPDATE items SET length = ?, width = ?, height = ?, note = ?, group_id = ?, item_order = ? WHERE id = ?',
            (length, width, height, note, group_id, int(item_order), item_id)
        )
    else:
        cursor.execute(
    ...
    cursor.execute('SELECT item_order FROM items WHERE id = ?', (item_id,))
    final_order = cursor.fetchone()['item_order']
    conn.close()
